...
def flatten(aList):
    ...
    for item in aList:
        if type(item) != list:  # If the item is not a list, add it directly to newList
            newList.append(item)
        else:  # If the item is a list, recursively flatten it and extend newList with the result
